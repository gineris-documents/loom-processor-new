...
def process_video():
    ...
    try:
        ...
        # Extract frames
        print(f"Extracting frames with interval {interval} seconds...")
        ...
        try:
            ...
            if not audio_path:
                print(f"Audio extraction warning (continuing anyway): {audio_error}")
        except Exception as audio_ex:
            print(f"Audio extraction exception (continuing anyway): {audio_ex}")
        ...
        # Create a folder for this SOP in Drive
        sop_folder_name = f"{title}_{timestamp}"
        sop_folder = None
        ...
        # Create SOP folder in Drive
        try:
            folder_metadata = {
                'name': sop_folder_name,
                'mimeType': 'application/vnd.google-apps.folder',
                'parents': [folder_id] if folder_id else [drive_id]
            }
            
            sop_folder = drive_service.files().create(
                body=folder_metadata,
                fields='id,name,webViewLink',
                supportsAllDrives=True
            ).execute()
            
            print(f"Created SOP folder: {sop_folder}")
        except Exception as folder_ex:
            print(f"Error creating SOP folder: {folder_ex}")
            # Continue using the parent folder directly
        
        # Use the new folder ID if created, otherwise use the original folder/drive ID
        upload_folder_id = sop_folder.get('id') if sop_folder else (folder_id or drive_id)
        
        # Upload files to Drive
        uploaded_files = []
        
        # Function to upload a file and add to the list
            ...
            try:
                ...
                if drive_id:
                    ...
                    file_info, error = upload_to_shared_drive(
                        file_path, drive_id, upload_folder_id, file_name
                    )
                else:
                    # Use regular Drive
                    file_info, error = upload_to_drive(
                        file_path, upload_folder_id, file_name
                    )
        ...
        video_file = upload_file_to_drive(video_path, f"{title}_video.mp4")
        
        # Upload frames
        print("Uploading frames to Drive...")
        frame_files = sorted([f for f in os.listdir(frames_dir) if f.endswith('.jpg')])
        for frame_file in frame_files:
            frame_path = os.path.join(frames_dir, frame_file)
            upload_file_to_drive(frame_path, f"{title}_{frame_file}")
        
        # Upload audio if extracted
        if audio_path:
            print("Uploading audio to Drive...")
            upload_file_to_drive(audio_path, f"{title}_audio.wav")
        ...
        return jsonify({
            ...
            "title": title,
            "interval": interval,
            "folder": sop_folder if sop_folder else {"id": upload_folder_id},
            "files": uploaded_files
        })
